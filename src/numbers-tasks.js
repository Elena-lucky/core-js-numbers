...
function getAverage(value1, value2) {
  if (Number.isFinite(value1) && Number.isFinite(value2)) {
    ...
    return Number.isFinite(average) ? average : 1.7976931348623157e308;
  }
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const upperLine = x1 * x2 + y1 * y2;
  const vector1 = Math.sqrt(x1 * x1 + y1 * y1);
  const vector2 = Math.sqrt(x2 * x2 + y2 * y2);
  const cosVectors = upperLine / (vector1 * vector2);
  const angleBetweenVectors = Math.acos(cosVectors);
...
function roundToPowerOfTen(num, pow) {
  ...
  const roundedToPowerOfTen = Math.round(num / divisor) * divisor;
  return roundedToPowerOfTen;
}
...
function toNumber(value, def) {
  const newValey = Number(value);
  if (!Number.isNaN(newValey)) {
    return newValey;
  }
  return def;
}
...
function toPrecision(number, precision) {
  const newNumber = Number(number.toPrecision(precision));
